        ...
        <>
            <header className="sticky  top-0 z-10 bg-surface-dark backdrop-blur-md border-b border-slate-200 dark:border-slate-700">
              ...
              {navs.map((nav) => (
                    ...
                    <button className="md:hidden p-2 text-slate-600 dark:text-slate-300 cursor-pointer" onClick={handleDrawerOpen}>
            ...
            <Drawer isOpen={isDrawerOpen} onClose={handleDrawerClose}>
                <nav className="flex flex-col space-y-4">
                    {navs.map((nav) => (
                        <a key={nav.name} className="text-slate-400 hover:text-slate-200 transition-colors text-lg font-bold border-b border-slate-700 pb-2" href={nav.href}>
                            {nav.name}
                        </a>
                    ))}
                </nav>
