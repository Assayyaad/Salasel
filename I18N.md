...
2. **Update `app/static.ts`:**
   ...
   export const translationsMap: Readonly<Record<LanguageCode, Translations>> = Object.freeze({
     ...
     fr: require('../public/i18n/fr.json'), // Add French
   })
   ```
...
4. **Create `public/i18n/fr.json`:**
   Copy structure from `ar.json` and translate all keys according to `public/i18n/schema.json` schema.
...
The `public/i18n/schema.json` file defines all required translation keys using JSON Schema with descriptions. All language files must include these keys.

To add a new translation key:

1. Add it to `public/i18n/schema.json` schema with a descriptive comment
